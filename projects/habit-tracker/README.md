# Habit Tracker

A weekly habit tracker. Add habits, tick each day on a 7-day grid, and watch
your **streaks** and **weekly completion %**. Everything saves to your browser,
so it's there next time.

A single self-contained HTML page (Tailwind via CDN). Category: **personal /
productivity** — it demonstrates a localStorage-first private tool.

## Load it into App Studio

- **Load it straight in** — use [`habit-tracker.appstudio`](./habit-tracker.appstudio):
  **Share → Import**, or drop it into your `App Studio Projects` folder.
- **Copy-paste** — open [`habit-tracker.html`](./habit-tracker.html), copy it
  all, then **New → paste → Run App**.

## Notes

- Private and single-user: no accounts, no server. Data lives in `localStorage`
  (clear it with the in-app reset). **No runtime service dependency.**
- Made with the [personal](../../prompts/context/personal.md) category add-on.
- Keep the header comment and the source link at the top of the file. See the
  root [LICENSE](../../LICENSE) for the terms.
