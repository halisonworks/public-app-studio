...
![Strømkostnad screenshot](./screenshot.png)
...
Work out electricity use (kWh) and cost — from price and usage, or from an appliance wattage and runtime.

Practical everyday maths for Norwegian **matematikk 2P** (LK20) — for work and
home. **Norwegian UI.** A single self-contained HTML page (Tailwind via CDN).
Category: **Math · 2P praktisk**.

## Load it into App Studio

- **Load it straight in** — use [`stromkostnad.appstudio`](./stromkostnad.appstudio):
  **Share → Import**, or drop it into your `App Studio Projects` folder.
- **Copy-paste** — open [`stromkostnad.html`](./stromkostnad.html), copy it all, then
  **New → paste → Run App**.
