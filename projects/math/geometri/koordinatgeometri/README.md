...
![Koordinatgeometri screenshot](./screenshot.png)
...
Distance, midpoint and slope between two points in the coordinate plane.

Interactive geometry tool for Norwegian **matematikk 1P/2P** (LK20). **Norwegian UI.**
A single self-contained HTML page (Tailwind via CDN; hand-rolled SVG). Category:
**Math · Geometri**.

## Load it into App Studio

- **Load it straight in** — use [`koordinatgeometri.appstudio`](./koordinatgeometri.appstudio):
  **Share → Import**, or drop it into your `App Studio Projects` folder.
- **Copy-paste** — open [`koordinatgeometri.html`](./koordinatgeometri.html), copy it all, then
  **New → paste → Run App**.
